...
GO -- Erstellung der physikalischen Demotabelle in tempdb
    CREATE TABLE dbo.transtest (wert TINYINT);
GO -- Start der Transaktion
    BEGIN TRANSACTION;
BEGIN TRY
INSERT INTO dbo.transtest (wert)
VALUES (1);
INSERT INTO dbo.transtest (wert)
VALUES (2);
...
PRINT 'Erfolgreich';
COMMIT TRANSACTION;
END TRY BEGIN CATCH PRINT 'FEHLER';
IF @@TRANCOUNT > 0 BEGIN ROLLBACK TRANSACTION;
END
END CATCH;
GO -- Verifikation des Tabelleninhalts
SELECT *
FROM dbo.transtest;
GO -- Bereinigung
...
END
ELSE BEGIN PRINT 'Kein Rollback nötig, da keine aktive Transaktion existiert.';
END
END CATCH;
GO -- Verifikation
SELECT *
